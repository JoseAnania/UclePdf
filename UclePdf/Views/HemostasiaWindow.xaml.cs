...
{
    public partial class HemostasiaWindow : Window
    {
        private static readonly Regex NumericRegex = new Regex("^[0-9.,]+$", RegexOptions.Compiled);

        public HemostasiaWindow() : this(new HemostasiaViewModel()) { }

        public HemostasiaWindow(HemostasiaViewModel vm)
        {
            InitializeComponent();
            DataContext = vm;
        ...
        {
            if (DataContext is HemostasiaViewModel vm)
            {
                vm.Confirm();
            }
            DialogResult = true;
