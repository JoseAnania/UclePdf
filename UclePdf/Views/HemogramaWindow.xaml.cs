...
{
    public partial class HemogramaWindow : Window
    {
        private static readonly Regex NumericRegex = new Regex("^[0-9.,]+$", RegexOptions.Compiled);

        public HemogramaWindow() : this(new HemogramaViewModel()) { }

        public HemogramaWindow(HemogramaViewModel vm)
        {
            InitializeComponent();
            DataContext = vm;
        ...
        {
            if (DataContext is HemogramaViewModel vm)
            {
                vm.Confirm();
            }
            DialogResult = true;
