...
public partial class IonogramaWindow : Window
{
    private static readonly Regex NumericRegex = new Regex("^[0-9.,]+$", RegexOptions.Compiled);

    public IonogramaWindow() : this(new IonogramaViewModel()) { }

    public IonogramaWindow(IonogramaViewModel vm)
    {
        InitializeComponent();
        DataContext = vm;
    ...
    {
        if (DataContext is IonogramaViewModel vm)
        {
            vm.Confirm();
        }
        DialogResult = true;
