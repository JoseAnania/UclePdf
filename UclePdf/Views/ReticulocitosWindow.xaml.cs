...
public partial class ReticulocitosWindow : Window
{
    private static readonly Regex NumericRegex = new Regex("^[0-9.,]+$", RegexOptions.Compiled);

    public ReticulocitosWindow() : this(new ReticulocitosViewModel()) { }

    public ReticulocitosWindow(ReticulocitosViewModel vm)
    {
        InitializeComponent();
        DataContext = vm;
    ...
    {
        if (DataContext is ReticulocitosViewModel vm)
        {
            vm.Confirm();
        }
        DialogResult = true;
